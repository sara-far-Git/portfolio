...
והשאר ל"פרויקטים אישיים". פרויקט עם `links.demo` מקבל מסגרת דפדפן עם צילום מסך
סטטי (`shot`). פרויקט בלי אתר חי מקבל במקום זה פאנל `flow` שמתאר את ה-pipeline.

הבילד מייצר HTML סטטי (`dist/index.html` בעברית, `dist/en.html` באנגלית) בלי
חבילת React — כדי שקורא, קריולר או מודל שמושך את ה-URL יקבלו את כל התוכן בלי
להריץ JavaScript. ב-`npm run dev` האתר עדיין רץ כ-SPA עם מתג שפה.
